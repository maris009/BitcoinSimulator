...
def client_transaction(message1):
    ...
    clientSocketB.bind((serverName, client_port_b))
    message = message1
    clientSocketB.sendto(message.encode(), (serverName, serverPortF2))
    clientSocketB.close()
...
def transaction():
    ...
    if new_balance >= 0:
        with open('Unconfirmed_TB.txt', 'a') as U:
            U.write(Tx + '\n')
...
def update_balance(account, new_balance, confirmed_balance):
    ...
    with open('BalanceB.txt', 'r+') as f:
        if account == 'B0000001':
            f.seek(0)
            f.write(update_amount)
        elif account == 'B0000002':
            f.seek(26)
            f.write('\n'+update_amount)
    f.close()
...
def print_current_balance():
    num_hex, confirmed_balance = current_balance('B0000001')
    print(f'B0000001 current balance is {num_hex}')
    num_hex, confirmed_balance = current_balance('B0000002')
    print(f'B0000002 current balance is {num_hex}')


def print_unconfirmed_transactions():
    with open('Unconfirmed_TB.txt') as f:
        for line in f:
            count = 0
            for word in line.split():
                print(word)
...
def print_confirmed_transactions():
    ...
    index = 0
    for line in reversed(list(open('Confirmed_TB.txt'))):
        if index < count:
            print(line.rstrip())
            index = index + 1
        else:
...
def printing_block_chain():
    ...
    client_port_b = 35000
    clientSocketB = socket(AF_INET, SOCK_DGRAM)
    clientSocketB.bind(('', client_port_b))
...
def main():
    ...
    while true:
        ...
        if choice == '1':
            ...
            print("2. Current balances for each account B.")
